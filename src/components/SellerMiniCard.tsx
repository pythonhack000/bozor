...
export function SellerMiniCard({ seller }: { seller: Seller }) {
  const { t, tl } = useI18n();
  const { user } = useAuth();
  ...
  return (
    <div className="rounded-lg border border-border bg-surface p-4">
      ...
      <button
        ...
        className="mt-3 flex w-full items-center justify-center gap-1.5 rounded-lg border border-border py-2 text-xs font-medium text-foreground/90 transition hover:border-brand/40 hover:text-brand"
      >
